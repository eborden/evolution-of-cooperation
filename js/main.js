    ...
    switch (e.data.type) {
      ...
      case 'playby':
        var population = e.data.message.length,
            ...
            widths = _.reduce(_.map(moralities(), (function(x) {
              return x.name;
            })), function(o, name) {
              o[name] = data[name] ? Math.floor(data[name].length / population * 100) : 0;
              return o;
            }, {}),
            total = _.reduce(widths, (function(x, y) {
              return x + y;
            }));
        if (total !== 100) {
          widths[_.keys(widths)[0]] += 100 - total;
        }
        log(_.reduce(widths, (function(x, y) {
          return x + y;
        })));
        _.forEach(moralities().map((function(x) {
          return x.name;
        })), function(name) {
        ...
        document.getElementById('survive' + e.data.id).innerHTML = [loaders[e.data.id]('evolving'), _.map(data, (function(x, k) {
          return [k, x.length].join(': ');
        })).sort().join('; '), 'generation:', e.data.generation].join(' ');
        break;
      case 'result':
...
function coalesce() {
  return _.find(Array.prototype.slice.call(arguments, 0), (function(x) {
...
function setupSimulation() {
  ...
  morlts.forEach(function(m) {
    var d = create({
      tag: 'div',
      className: m.name + ' ' + getColor()
    }, create({
      tag: 'h2',
      text: camelToSpace(m.name)
    }), create({tag: 'article'}, create({
      tag: 'p',
      text: m.description
    })));
  ...
  simulations[i].appendChild(create({
    tag: 'var',
    text: 0
  }));
  simulationsEl.appendChild(simulations[i]);
  consoleEl.appendChild(create({
    tag: 'p',
    id: 'survive' + i
  }));
  loaders[i] = loader();
...
function resize() {
  setTimeout(function() {
    ...
    _.toArray(document.getElementsByTagName('section')).forEach((function(x) {
      return x.style.minHeight = window.innerHeight + 'px';
    }));
  }, 0);
}
var scrollTop = (function() {
  ...
  return function scrollTop(position, speed) {
    ...
    scrollSteps = _.each(repeat(0, 5), (function(x) {
      return x;
    }));
  };
}());
